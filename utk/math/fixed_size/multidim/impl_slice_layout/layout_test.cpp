...
struct nothing_fixed_234
{
    ...
    typedef multidim_slice_layout< old_layout, index_vector< 2,3,4 > > layout;
};
...
BOOST_FIXTURE_TEST_SUITE( nothing_fixed, nothing_fixed_234 )
  ...
  BOOST_AUTO_TEST_CASE( size )
  {
    ...
    const size_type total_size = layout::total_size;
    BOOST_CHECK_EQUAL( total_size, 24 );

    std::array< size_type, 3 > size_array = meta::integral::make_array< typename layout::sizes >::value;
    BOOST_CHECK_EQUAL( size_array[0], 2 );
  }

  BOOST_AUTO_TEST_CASE( index_offset )
  {
    const stride_type old_offset_111 = layout::index_offset( 1,1,1 );
    const stride_type offset_111 = layout::index_offset( 1,1,1 );
...
{
    typedef multidim_layout< size_vector<2,3,4> > old_layout;
    typedef multidim_slice_layout< old_layout, index_vector< 1,3,4 > > layout;
};

BOOST_FIXTURE_TEST_CASE( unmask_index, first_fixed_234 )
{
  const index_type unmask0 = helpers::unmask_index< layout::visibility_mask, 0 >::value;
  BOOST_CHECK_EQUAL( unmask0, 1 );

  const index_type unmask1 = helpers::unmask_index< layout::visibility_mask, 1 >::value;
...
BOOST_FIXTURE_TEST_SUITE( first_fixed, first_fixed_234 )
  ...
  {
    index_type fixed0 = meta::integral::at< typename layout::sizes, 0 >::value;
    BOOST_CHECK_EQUAL( fixed0 , 3 );

    index_type fixed1 = meta::integral::at< typename layout::sizes, 1 >::value;
    BOOST_CHECK_EQUAL( fixed1 , 4 );
  }

  BOOST_AUTO_TEST_CASE( index_offset )
  {
    const stride_type fixed_11 = layout::index_offset( 1,1 );
    BOOST_CHECK_EQUAL( fixed_11, 5 );
  }

  BOOST_AUTO_TEST_CASE( static_offset )
  {
    const stride_type fixed1 = layout::static_offset();
