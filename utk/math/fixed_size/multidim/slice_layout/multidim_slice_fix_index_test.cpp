/*  multidim_slice_fix_index_test.cpp - Copyright Peter Urban 2009
...
#define BOOST_TEST_MODULE multidim_slice_layout fix_index
...
struct slice__fix_unmasked_index
{
  ...
  typedef multidim_slice_layout< old_layout, index_vector< 1,3,4 > > slice_layout;
  typedef typename fix_unmasked_index< slice_layout, 0,1 >::type layout;
  // fixed 0->1 (overwrite)
};
...
BOOST_FIXTURE_TEST_SUITE( check_slice__fix_unmasked_index, slice__fix_unmasked_index )
  ...
  {
    const stride_type offset_static = layout::static_offset();
    BOOST_CHECK_EQUAL( offset_static, 12 );
  }

BOOST_AUTO_TEST_SUITE_END()

//---| fix_index ( multidim_slice_layout ) |---------------------------/

struct slice__fix_index
{
  typedef multidim_layout< size_vector< 2,3,4 > > old_layout;
  typedef multidim_slice_layout< old_layout, index_vector< 1,1,4 > > layout;
  //typedef typename fix_index< old_layout, 0,1 >::type slice_layout;
  //typedef typename fix_index< slice_layout, 0,1 >::type layout;
  // fixed 0->1, 1->1
};
...
//---| fix_index ( multidim_layout ) |---------------------------------/

struct multidim__fix_index
{
  typedef multidim_layout< size_vector< 2,3,4 > > old_layout;
  typedef typename fix_index< old_layout, 0,1 >::type layout;
  // fixed 0->1
};

BOOST_FIXTURE_TEST_SUITE( check_multidim__fix_index, multidim__fix_index )
