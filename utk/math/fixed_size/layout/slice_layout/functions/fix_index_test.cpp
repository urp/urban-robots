...
#define BOOST_TEST_MODULE layout::slice_layout fix_index
...
{
  typedef generic_layout< size_vector< 2,3,4 > > old_layout;
  //typedef slice_layout< old_layout, index_vector< 1,1,4 > > generic_layout4;
  typedef typename fix_index< old_layout, 0,1 >::type slice_layout;
  typedef typename fix_index< slice_layout, 0,1 >::type generic_layout4;
  // fixed 0->1, 1->1
};
...
BOOST_FIXTURE_TEST_SUITE( check_slice__fix_index, slice__fix_index )
  ...
  BOOST_AUTO_TEST_CASE( check_order )
  {
    const size_type order = generic_layout4::order;
    BOOST_CHECK_EQUAL( order, 1 );
  }
  BOOST_AUTO_TEST_CASE( check_size )
  {
    const size_type size0 = meta::integral::at< typename generic_layout4::sizes, 0 >::value;
    BOOST_CHECK_EQUAL( size0, 4 );
  }
  BOOST_AUTO_TEST_CASE( check_stride )
  {
    const stride_type stride0 = meta::integral::at< typename generic_layout4::strides, 0 >::value;
    BOOST_CHECK_EQUAL( stride0, 1 );
  }
  BOOST_AUTO_TEST_CASE( check_static_offset )
  {
    const stride_type offset_static = generic_layout4::static_offset();
...
//---| fix_index ( generic_layout ) |---------------------------------/

struct generic_layout_fix_index
{
  typedef generic_layout< size_vector< 2,3,4 > > old_layout;
...
BOOST_FIXTURE_TEST_SUITE( check__fix_index, generic_layout_fix_index )
