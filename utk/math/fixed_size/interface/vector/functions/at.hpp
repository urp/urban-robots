...
namespace utk
{
  namespace math
  {
	...
	namespace vector
	{
	  ...
	  ValueType& at( interface< ValueType, Size >& interface, size_t index ) throw( std::out_of_range )
	  {
	  ...
	  template< typename ValueType, size_t Size >
	  const ValueType& at( const interface< ValueType, Size >& interface, size_t index ) throw( std::out_of_range )
	  {
	    if( index >= interface.size )
	      throw std::out_of_range( "index "
				      + boost::lexical_cast<std::string>( index )
