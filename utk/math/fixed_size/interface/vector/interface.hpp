...
# include "utk/math/fixed_size/interface/vector/functions/storage_traits.hpp"
      ...
      namespace interface
      {
        namespace vector
        {
          ...
          class interface
          {
              ...
              // leave memory unassigned (use interface<ValueType,Size>::invalid if you need it)
            ...
            public:
              ...
              type& operator= ( const ValueType& scalar )
              {
              ...
              //-------return true if all elements compare as equal to ValueType(true)
              template< typename TT = ValueType, size_type C = Size
                      , typename = typename std::enable_if< (C > 1) && ( std::is_same<ValueType,bool>::value ) >::type >
              operator bool() const
              {
                return std::all_of( begin(), end(), std::bind2nd( std::equal_to<ValueType>(), true ) );
              }
          ...
          template< typename ValueType, size_type Size >
          const typename interface< ValueType, Size >::type interface< ValueType, Size >::invalid = interface< ValueType, Size >( nullptr );

          //---| size

          template< typename ValueType, size_type Size >
          constexpr size_type interface< ValueType, Size >::size;
