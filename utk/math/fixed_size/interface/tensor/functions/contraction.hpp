/*  contracton.hpp - Copyright Peter Urban 2012-2013
...
# include "utk/math/fixed_size/interface/tensor/operators/plus.hpp"
...
namespace utk
{
  namespace math
  {
	...
	namespace tensor
	{
 	  ...
 	  // !!! TODO: make generic !!! (specialize to include variance checks)
	  ...
	  struct contracted< interface< ValueType, StorageTag, Layout >, Index, DualIndex >
	  {
	    ...
	    typedef typename layout::remove_index< Layout, DualIndex >::type dual_removed;
	    typedef typename layout::remove_index< dual_removed, Index >::type contracted_layout;

	    typedef typename layout::replace_strides< contracted_layout >::type new_layout;
	  ...
	  struct assign_contraction
	  {
	    ...
	    static auto apply( const Tensor& A, Result& result ) -> void
	    {
	      ...
	      typedef typename layout::slice_layout< typename Tensor::layout, index_mask > new_layout;
