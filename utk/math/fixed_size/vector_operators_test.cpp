...
#define BOOST_TEST_MODULE vector operators test
#include <boost/test/unit_test.hpp>

using namespace utk::math::fixed_size;

BOOST_AUTO_TEST_CASE( vecn_comparison_operators )
{
  double data_one[] { 1., 2., 3. };
  const vector_interface<double,3> one( data_one );
  double data_two[] { 1., 2., 3. };
  const vector_interface<double,3> two( data_two );

  double data_gt_one[] { 1., 2.2, 3. };
  const vector_interface<double,3> gt_one( data_gt_one );

  double data_gt_all[] { 1.1, 2.2, 3.3 };
  const vector_interface<double,3> gt_all( data_gt_all );
  
  double data_lt_one[] { 1., 0.2, 3. };
  const vector_interface<double,3> lt_one( data_lt_one );

  double data_lt_all[] { 0.1, 0.2, 0.3 };
  const vector_interface<double,3> lt_all( data_lt_all );

  bool data_true_all[]  { true, true, true };
  const vector_interface<bool,3>  true_all( data_true_all );

  bool data_false_all[] { false, false, false };
  const vector_interface<bool,3> false_all( data_false_all );

  bool data_true_one[]  { false, true, false };
  const vector_interface<bool,3>  true_one( data_true_one );

  bool data_false_one[] { true, false, true };
  const vector_interface<bool,3> false_one( data_false_one );
...
BOOST_AUTO_TEST_CASE( vecn_arithmetic_operators )
{
  ...
  const vector_interface<double,3> a_negative( data_a_negative );
  BOOST_CHECK_EQUAL( -a, a_negative );

  // plus
  double data_a_plus_b[] { 1.+2., 2.+4., 3.+6. };
  const vector_interface<double,3> a_plus_b( data_a_plus_b );
  double data_a_plus_s[] { 1.+s, 2.+s, 3.+s };
  const vector_interface<double,3> a_plus_s( data_a_plus_s );
  vector_array<double,3> a_plus(a);
  vector_array<double,3> s_plus(a);
  ...
  const vector_interface<double,3> a_minus_b( data_a_minus_b );
  double data_a_minus_s[] { 1.-s, 2.-s, 3.-s };
  const vector_interface<double,3> a_minus_s( data_a_minus_s );
  vector_array<double,3> a_minus(a);
  vector_array<double,3> s_minus(a);  
  ...
  const vector_interface<double,3> a_mult_b( data_a_mult_b );
  double data_a_mult_s[] { 1.*s, 2.*s, 3.*s };
  const vector_interface<double,3> a_mult_s( data_a_mult_s );
  vector_array<double,3> a_mult(a);
  vector_array<double,3> s_mult(a);
  ...
  const vector_interface<double,3> a_div_b( data_a_div_b );
  double data_a_div_s[] { 1./s, 2./s, 3./s };
  const vector_interface<double,3> a_div_s( data_a_div_s );
  vector_array<double,3> a_div(a);
  vector_array<double,3> s_div(a);
