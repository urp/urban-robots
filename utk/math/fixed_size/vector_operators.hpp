...
namespace utk
{
  namespace math
  {
	namespace fixed_size
	{
      ...
      std::ostream&	operator<<(std::ostream& os,const vector_interface<T,Cnt>& v)	
      { utk::io::sequence_to_stream( os<<'(' ,v.begin(),v.end(),",");
        return os<<')';
      }
  
      template<class T,size_t Cnt>
      std::istream&	operator>>(std::istream& is,vector_interface<T,Cnt>& v)	
	  ...
	  vector_array<bool,Cnt>	operator==( const vector_interface<T,Cnt>& a, const vector_interface<T,Cnt>& b )
	  { 
	    vector_array<bool,Cnt> r;
      ...
      template<typename T,size_t Cnt>
	  bool    operator==( const vector_interface<T,Cnt>& v, const T& s )
	  {
	  ...
	  template<typename T,size_t Cnt>
	  vector_array<T,Cnt>	operator! (const vector_interface<T,Cnt>& o)
	  {
	    vector_array<bool,Cnt>	res;
	  ...
	  template<typename T,size_t Cnt>
	  vector_array<T,Cnt>	operator- (const vector_interface<T,Cnt>& v)
	  { 
	    vector_array<T,Cnt> res(v);
	    return res *= -T(1);
	  }
	  
	  template<typename T,size_t Cnt>
	  vector_array<T,Cnt>	operator+ ( const vector_interface<T,Cnt>& a, const vector_interface<T,Cnt>& b )
	  { 
	    vector_array<T,Cnt> r;
	  ...
	  template<typename T,size_t Cnt>
	  vector_array<T,Cnt>	operator+ ( const vector_interface<T,Cnt>& a, const T& s )
	  { 
	    vector_array<T,Cnt> r;
	  ...
	  template<typename T,size_t Cnt>
	  vector_interface<T,Cnt>&	operator+= ( vector_interface<T,Cnt>& a, const vector_interface<T,Cnt>& b)
	  { 
	  ...
	  template<typename T,size_t Cnt>
	  vector_interface<T,Cnt>&	operator+= ( vector_interface<T,Cnt>& v, const T& s)
	  {
