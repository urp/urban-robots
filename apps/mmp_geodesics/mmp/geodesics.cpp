...
: surf(surface)
, m_source(source)
, vertex_labels( surface.num_vertices() )
, windows( surface.get_property_map<boost::edge_index_t>() )
, event_queue(), m_max_distance( 0 )
{
...
std::pair< Geodesics::winlist_t::iterator, ev_pair_t >  Geodesics::delete_window( winlist_t::iterator loc, winlist_t& wlist )
{
  ...
  EventPoint::Grabber< event_queue_t::iterator, EventPoint::FRONTIER > evgrab( event_queue.begin(), event_queue.end(), *loc );
...
void    Geodesics::propagate_window( EventPoint& ev)
{
  ...
  // the two possible edges on which new windows can be created
  ...
  else
  {
    ...
    // check: either projected coordinates are ordered or located on different edges
    // TODO: reenable assert( bound0 <= bound1 || (edge0 == e1.descriptor() && edge1 == e2.descriptor()) );

    // window vanished
    // TODO: reenable: check bounds equal ( see assertion above )
