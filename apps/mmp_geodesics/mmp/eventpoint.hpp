...
namespace mmp
{
  ...
  class EventPoint
  {
	...
	if( colinear )
	  set_colinear_flag< Side >();
	else
	  unset_colinear_flag< Side >();
      }

      template< side_t Side >
      void set_colinear_flag()
      { m_mask |= Side == LEFT ? LEFT_COLINEAR : RIGHT_COLINEAR; }

      template< side_t Side >
      void unset_colinear_flag()
      {
	set_colinear_flag<Side>();
	m_mask ^= Side == LEFT ? LEFT_COLINEAR : RIGHT_COLINEAR;
      }
	...
	# endif
      ...
      template< side_t Side >
      EventPoint* colinear() const
      { return  m_mask & ( Side == LEFT ? LEFT_COLINEAR : RIGHT_COLINEAR ) ? adjacent<Side>() : 0; }
