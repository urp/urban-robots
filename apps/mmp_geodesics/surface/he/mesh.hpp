...
namespace he
{
  ...
  class Mesh
  {
    ...
    public:
      ...
      const vertices_size_type	num_vertices() const
      { return boost::num_vertices(m_graph); }

      const edges_size_type	num_edges() const
      { return boost::num_edges(m_graph); } // TODO: profile against m_num_half_edges

      const edges_size_type&	num_full_edges() const
      { return m_num_full_edges; }

      const faces_size_type	num_faces() const
      { return m_face_vec.size(); }

      const std::pair< vertex_handle_iterator, vertex_handle_iterator >
  ...
  class PrimitiveSlot
  {
      friend typename Storage::vertex_handle_iterator;
      friend typename Storage::edge_handle_iterator;
      friend typename Storage::face_handle_iterator;
  ...
  class DefaultEdgeHandle : public PrimitiveSlot< typename Storage::edge_descriptor, Storage >
  {
	...
	return face_handle( m_he.face, this->mesh());
      ...
      index_t index() const
      {
        return this->mesh().template get< boost::edge_index_t>( this->descriptor() );
      }
